...
contract NFTPoolLockAndRelease is CCIPReceiver, OwnerIsCreator {
    ...
    function lockAndSendNFT(uint256 tokenId, address newOwner, uint64 chainSelector, address recevier) public returns(bytes32){
        //1:transfer nft from source to pool
        nft.transferFrom(msg.sender, address(this), tokenId);
        
        //construct data to be sent
        bytes memory payload = abi.encode(tokenId , newOwner);
        bytes32 messageId = sendMessagePayLINK(chainSelector, recevier, payload);
        return messageId;
    }
        ...
        override
    {
        ...
        //tranfer nft from pool to source
